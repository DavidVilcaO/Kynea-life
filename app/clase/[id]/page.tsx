  ...
  return (
    <div className="min-h-screen bg-neutral-50">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <Link href="/buscar" className="flex items-center gap-2 text-sm text-neutral-500 hover:text-neutral-700 transition-colors">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="relative rounded-xl overflow-hidden">
              ...
              <div className="absolute top-4 left-4 flex gap-2">
                ...
                <span className="text-xs font-semibold px-3 py-1.5 rounded-full bg-pink-100 text-pink-600">
            ...
            {/* Title section */}
            <div>
              <p className="text-sm text-pink-600 font-semibold uppercase tracking-wide">{cls.style} · {cls.level}</p>
              <h1 className="text-2xl md:text-3xl font-black text-neutral-900 mt-1">{cls.title}</h1>
              <p className="text-neutral-500 mt-1">por <span className="text-neutral-700 font-medium">{cls.teacher.name}</span></p>
            </div>

            {/* Description */}
            <div className="bg-white rounded-xl p-6 shadow-sm border border-neutral-100">
              <h2 className="font-bold text-neutral-900 mb-3">Sobre esta clase</h2>
              <p className="text-neutral-600 text-sm leading-relaxed">{cls.fullDescription}</p>

              {cls.whatYouLearn && cls.whatYouLearn.length > 0 && (
                <div className="mt-4">
                  <h3 className="font-semibold text-neutral-900 mb-2 text-sm">¿Qué aprenderás?</h3>
              ...
              {cls.forWhom && (
                <div className="mt-4 p-3 bg-neutral-50 rounded-xl">
                  <p className="text-xs text-neutral-700 font-semibold mb-1">¿Para quién es?</p>
                  <p className="text-sm text-neutral-700">{cls.forWhom}</p>
                </div>
              )}
            </div>
            ...
            {/* Schedule */}
            <div className="bg-white rounded-xl p-6 shadow-sm border border-neutral-100">
              <h2 className="font-bold text-neutral-900 mb-4">Horarios</h2>
              <div className="space-y-3">
                {cls.timeSlots.map((slot, i) => (
                  <div key={i} className="flex items-center gap-3 p-3 bg-neutral-50 rounded-xl">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-neutral-900">{slot.days.join(', ')}</p>
              ...
              {cls.startDate && (
                <div className="mt-3 flex gap-4 text-sm text-neutral-500">
                  <span>Inicio: <strong className="text-neutral-700">{cls.startDate}</strong></span>
                  {cls.endDate && <span>Fin: <strong className="text-neutral-700">{cls.endDate}</strong></span>}
                </div>
              )}
              <div className="mt-3 flex items-center gap-2 text-sm">
                ...
                <span className="text-neutral-600">
                  {(cls.availableSpots ?? 0) > 0
                    ? <><strong className="text-neutral-900">{cls.availableSpots}</strong> cupos disponibles de {cls.maxSpots}</>
            ...
            {/* Location */}
            <div className="bg-white rounded-xl p-6 shadow-sm border border-neutral-100">
              <h2 className="font-bold text-neutral-900 mb-4">Ubicación</h2>
              <div className="flex items-center gap-1 text-sm text-neutral-500 mb-1">
                <span className="font-semibold text-neutral-800">{cls.modality}</span>
              </div>
              {cls.modality !== 'Online' && (
                <>
                  ...
                  {/* Simulated map */}
                  <div className="mt-4 rounded-xl overflow-hidden bg-neutral-100 h-40 relative">
                    <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, #e8f4f8 0%, #d1e8f0 100%)' }}>
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="bg-neutral-900 text-white rounded-full w-8 h-8 flex items-center justify-center shadow-lg">
                  ...
                  {cls.mapsUrl && (
                    <a href={cls.mapsUrl} target="_blank" rel="noopener noreferrer"
                      className="mt-3 flex items-center gap-2 text-sm text-neutral-900 font-medium hover:underline">
            ...
            {/* Requirements */}
            <div className="bg-white rounded-xl p-6 shadow-sm border border-neutral-100">
              <h2 className="font-bold text-neutral-900 mb-4">Recomendaciones</h2>
              <div className="grid sm:grid-cols-2 gap-4">
                {cls.footwear && (
                  <div>
                    <p className="text-xs text-neutral-500 font-semibold mb-1">Calzado</p>
                    <p className="text-sm text-neutral-700">👟 {cls.footwear}</p>
                  </div>
                )}
                {cls.clothing && (
                  <div>
                    <p className="text-xs text-neutral-500 font-semibold mb-1">Ropa</p>
                    <p className="text-sm text-neutral-700">👗 {cls.clothing}</p>
                  </div>
                )}
                {cls.prerequisites && (
                  <div>
                    <p className="text-xs text-neutral-500 font-semibold mb-1">Requisitos previos</p>
                    <p className="text-sm text-neutral-700">📋 {cls.prerequisites}</p>
                  </div>
                )}
                {cls.ageGroup && (
                  <div>
                    <p className="text-xs text-neutral-500 font-semibold mb-1">Edad</p>
                    <p className="text-sm text-neutral-700">🎂 {cls.ageGroup}</p>
                  </div>
                )}
                {cls.toBring && cls.toBring.length > 0 && (
                  <div className="sm:col-span-2">
                    <p className="text-xs text-neutral-500 font-semibold mb-2">Qué llevar</p>
                    <div className="flex flex-wrap gap-2">
                      {cls.toBring.map(item => (
                        <span key={item} className="text-xs bg-neutral-100 text-neutral-600 px-3 py-1 rounded-full">
                          {item}
                        </span>
                      ))}
                    </div>
            ...
            {/* Teacher profile */}
            <div className="bg-white rounded-xl p-6 shadow-sm border border-neutral-100">
              <h2 className="font-bold text-neutral-900 mb-4">Profesor / Academia</h2>
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <h3 className="font-bold text-neutral-900">{cls.teacher.name}</h3>
                  <p className="text-xs text-neutral-500 capitalize mb-2">{cls.teacher.type} · {cls.teacher.experience} años de experiencia</p>
                  {cls.teacher.rating && (
                  ...
                  <p className="text-sm text-neutral-600">{cls.teacher.bio}</p>
            ...
            {otherClasses.length > 0 && (
              <div>
                <h2 className="font-bold text-neutral-900 mb-4">Más clases de {cls.teacher.name}</h2>
                <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="md:col-span-1">
            <div className="bg-white rounded-xl p-6 shadow-sm border border-neutral-100 sticky top-24">
              ...
              <div className="flex gap-2">
                <button className="flex-1 flex items-center justify-center gap-2 border border-neutral-200 text-neutral-600 hover:border-neutral-900 hover:text-neutral-900 text-sm font-medium py-2.5 rounded-xl transition-colors">
      ...
      {/* Mobile sticky CTA */}
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-neutral-100 px-4 py-3 z-50">
        <div className="flex items-center gap-3">
          <div>
            <p className="text-lg font-black text-neutral-900">{formatPrice(cls.priceType, cls.price, cls.currency)}</p>
            <p className="text-xs text-neutral-500">{cls.availableSpots} cupos disponibles</p>
          </div>
          <a
