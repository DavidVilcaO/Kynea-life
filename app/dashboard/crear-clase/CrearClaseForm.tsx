...
import {
  ...
  Upload, MapPin, Monitor, Layers, Loader2, X,
} from 'lucide-react';
...
export default function CrearClaseForm({ classId, editClass }: Props) {
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <Pill key={opt.value} active={form.recurrence === opt.value} onClick={() => set('recurrence', opt.value)}>
              {opt.label}
            </Pill>
          ))}
        </div>
            ...
            <div key={i} className="border border-neutral-200 rounded-xl p-4 space-y-4 bg-neutral-50/50">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="time" className="input" value={slot.startTime} onChange={e => updateSlot(i, 'startTime', e.target.value)} />
                </div>
                <div>
                  <p className="text-xs font-semibold text-neutral-500 mb-1.5">Hora fin</p>
                  <input type="time" className="input" value={slot.endTime} onChange={e => updateSlot(i, 'endTime', e.target.value)} />
                </div>
              </div>
              {slot.days.length > 0 && (
                <p className="text-xs text-neutral-600 bg-white px-3 py-2.5 rounded-lg border border-neutral-200">
                  Esta clase se dicta los <span className="font-semibold">{slot.days.join(', ')}</span> de {slot.startTime} a {slot.endTime}
                </p>
              )}
            </div>
          ))}
        ...
        <div className="space-y-4 border border-neutral-200 rounded-xl p-4 bg-neutral-50/50">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className="input" value={form.district} onChange={e => set('district', e.target.value)} placeholder="Ej: Miraflores" />
            </div>
          </div>
          <div>
            <FieldLabel>Dirección</FieldLabel>
            <input className="input" value={form.address} onChange={e => set('address', e.target.value)} placeholder="Av. Benavides 1234, piso 3" />
          </div>
          <div>
            <FieldLabel>Referencia</FieldLabel>
            <input className="input" value={form.reference} onChange={e => set('reference', e.target.value)} placeholder="Frente al parque Kennedy" />
          </div>
        </div>
      )}
      ...
      {form.modality !== 'Presencial' && (
        <div className="space-y-4 border border-neutral-200 rounded-xl p-4 bg-neutral-50/50">
          ...
          <div>
            ...
            <input className="input" value={form.accessLink} onChange={e => set('accessLink', e.target.value)} placeholder="https://zoom.us/j/..." />
  ...
  const renderStep2 = () => (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <div className="relative">
              ...
              <input type="number" className="input pl-10" value={form.price} onChange={e => set('price', e.target.value)} placeholder="0" min={0} />
            </div>
          </div>
          <div>
      ...
      <div>
        ...
        <input type="number" className="input" value={form.maxSpots} onChange={e => set('maxSpots', e.target.value)} placeholder="Ej: 20" min={1} />
      </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          {[
            { value: 'whatsapp', label: 'Contactar por WhatsApp', desc: 'Los alumnos te escriben directamente' },
            ...
            <label key={opt.value}
              className={`flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all ${
                form.reservationMode === opt.value ? 'border-neutral-900 bg-neutral-50' : 'border-neutral-200 hover:border-neutral-400'
              }`}>
              <input type="radio" name="reservationMode" value={opt.value}
                checked={form.reservationMode === opt.value} onChange={() => set('reservationMode', opt.value)}
      ...
      <div>
        ...
        <input className="input" value={form.clothing} onChange={e => set('clothing', e.target.value)} placeholder="Ej: Ropa cómoda y transpirable" />
      </div>
        ...
        <div className="flex flex-wrap gap-2">
          {['Agua', 'Toalla', 'Rodilleras', 'Mat', 'Otro'].map(opt => (
            <Pill key={opt} active={form.toBring.includes(opt)} onClick={() => {
              const list = form.toBring.includes(opt) ? form.toBring.filter(x => x !== opt) : [...form.toBring, opt];
              set('toBring', list);
            }}>{opt}</Pill>
          ))}
        </div>
  ...
  const renderStep3 = () => {
    const priceLabel = form.priceType === 'Gratis' ? 'Gratis' : form.price ? `${form.currency === 'PEN' ? 'S/' : '$'}${form.price} (${form.priceType})` : '—';
    const locationLabel = form.modality !== 'Online' ? [form.address, form.district, form.city].filter(Boolean).join(', ') || '—' : form.platform || '—';
    const slotsLabel = slots.map(s => `${s.days.join(', ')} · ${s.startTime}–${s.endTime}`).join(' | ') || '—';
