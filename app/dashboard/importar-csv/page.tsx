  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-black text-gray-900">Importar clases por CSV</h1>
          <p className="text-gray-500 text-sm mt-0.5">Sube un archivo CSV con tus clases, horarios, precios y ubicaciones</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-purple-50 border border-purple-100 rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
            ...
            <div className="flex-1">
              <h3 className="font-bold text-gray-900">¿Primera vez importando?</h3>
              <p className="text-sm text-gray-600 mt-0.5">Descarga nuestra plantilla CSV con las columnas correctas y ejemplos incluidos</p>
            </div>
            <button className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white font-semibold text-sm px-4 py-2.5 rounded-xl transition-colors whitespace-nowrap">
          ...
          <div
            ...
            className={`border-2 border-dashed rounded-2xl p-12 text-center cursor-pointer transition-all ${
              dragging ? 'border-purple-500 bg-purple-50' : 'border-gray-200 hover:border-purple-300 hover:bg-gray-50'
            }`}
          >
            <input
              ref={fileRef}
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-2">Arrastra tu archivo CSV aquí</h3>
            <p className="text-sm text-gray-500 mb-4">o haz clic para seleccionar</p>
            <button className="text-sm font-semibold text-purple-600 border border-purple-200 px-6 py-2.5 rounded-xl hover:bg-purple-50 transition-colors">
              Seleccionar archivo
            </button>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="grid sm:grid-cols-3 gap-4 mb-5">
              {[
                ...
                <div key={req.label} className="bg-gray-50 rounded-xl p-3 text-center">
                  <p className="text-xs text-gray-500">{req.label}</p>
                  <p className="font-bold text-gray-900 text-sm mt-0.5">{req.value}</p>
                </div>
              ))}
            </div>

            <h4 className="text-sm font-semibold text-gray-700 mb-2">Columnas del archivo</h4>
            <div className="flex flex-wrap gap-1.5">
              {CSV_COLUMNS.map(col => (
                <span key={col} className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full font-mono">{col}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-500">{MOCK_PREVIEW.length} filas detectadas</p>
            </div>
            <button onClick={() => setImportStep('upload')} className="text-gray-400 hover:text-gray-600">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-gray-100">
              <h3 className="font-bold text-gray-900">Vista previa de datos</h3>
            </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-gray-50 text-left">
                    ...
                    <th className="px-4 py-3 font-semibold text-gray-500">Título</th>
                    <th className="px-4 py-3 font-semibold text-gray-500">Estilo</th>
                    <th className="px-4 py-3 font-semibold text-gray-500">Nivel</th>
                    <th className="px-4 py-3 font-semibold text-gray-500">Días</th>
                    <th className="px-4 py-3 font-semibold text-gray-500">Hora</th>
                    <th className="px-4 py-3 font-semibold text-gray-500">Precio</th>
                    <th className="px-4 py-3 font-semibold text-gray-500">Estado</th>
                    <th className="px-4 py-3 font-semibold text-gray-500">Validación</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {MOCK_PREVIEW.map(row => (
                    <tr key={row.row} className={row.status === 'error' ? 'bg-red-50/40' : row.status === 'warning' ? 'bg-yellow-50/40' : ''}>
                      ...
                      <td className="px-4 py-3 text-gray-600">{row.estilo}</td>
                      <td className="px-4 py-3 text-gray-600">{row.nivel}</td>
                      <td className="px-4 py-3 text-gray-600">{row.dias}</td>
                      <td className={`px-4 py-3 ${row.hora_inicio === '25:00' ? 'text-red-500' : 'text-gray-600'}`}>
                        {row.hora_inicio}
                      </td>
                      <td className={`px-4 py-3 ${isNaN(Number(row.precio_monto)) ? 'text-red-500' : 'text-gray-600'}`}>
                        {row.precio_monto}
                      </td>
                      <td className="px-4 py-3">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                          row.estado === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'
                        }`}>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              ...
              className="text-sm font-medium px-5 py-3 border border-gray-200 rounded-xl text-gray-600 hover:border-gray-300 transition-colors"
            >
              Volver a subir
            </button>
            <button
              onClick={() => setImportStep('success')}
              className="flex-1 text-sm font-bold py-3 border-2 border-dashed border-purple-300 text-purple-600 rounded-xl hover:bg-purple-50 transition-colors"
            >
              Importar como borradores ({validRows + warningRows} clases)
            </button>
            <button
              onClick={() => setImportStep('success')}
              className="flex-1 text-sm font-bold py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-xl transition-colors"
            >
      ...
      {importStep === 'success' && (
        <div className="text-center py-16">
          ...
          <h2 className="text-2xl font-black text-gray-900 mb-2">¡Importación exitosa!</h2>
          <p className="text-gray-500 mb-2">Se importaron {validRows + warningRows} clases correctamente.</p>
          {errorRows > 0 && (
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center mt-8">
            <Link
              ...
              className="text-sm font-bold px-6 py-3 bg-purple-600 hover:bg-purple-700 text-white rounded-xl transition-colors"
            >
              Ver mis clases
            </Link>
            <button
              onClick={() => setImportStep('upload')}
              className="text-sm font-medium px-6 py-3 border border-gray-200 rounded-xl text-gray-600 hover:border-gray-300 transition-colors"
            >
