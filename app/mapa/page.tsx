...
import { formatPrice, formatTimeSlots, getTypeLabel } from '@/lib/mockData';

export default function MapaPage() {
  const [selected, setSelected] = useState<DanceClass | null>(null);
  const [hoveredPin, setHoveredPin] = useState<string | null>(null);

  const classesWithCoords = mockClasses.filter(c => c.lat && c.lng && c.status !== 'draft');

  // Simulate a map with positioned pins on a styled background
  const mapBounds = {
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 relative bg-neutral-100 overflow-hidden">
          ...
          >
            ...
            {/* Pins */}
            {classesWithCoords.map(cls => {
              const pos = cls.lat && cls.lng ? toPercent(cls.lat, cls.lng) : null;
              if (!pos) return null;
              const isSelected = selected?.id === cls.id;
              const isHovered = hoveredPin === cls.id;
              return (
                <button
                  key={cls.id}
